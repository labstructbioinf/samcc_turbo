...
def select_minimal_angle_layer_set(layers_sets, best_layer_nb=1):
	...
	def check_layers_shape(layers_set):
		...
		#FIXME rebuild to accomodate searchLayerClass
...
def select_minimal_distance_layer_set(layers_sets):
	...
	#FIXME format docstring, rewrite docs for new code
	#FIXME rebuild to accomodate searchLayerClass
...
# 	# 		print('a-len', len(i))
#     #
# 	print('HELICES PTS SET', helices_pts_set)
...
# 	#print('helices_pts', helices_pts)
#
# 	first_layer = []
# 	for mdist in min_distance:
# 		points_set = []
# 		for helix_point in enumerate(mdist):
# 			# print('hlx point', helix_point)
# 			# print('hlx point', helices_pts[helix_point[0]][helix_point[1]])
...
#
# 	# FIXME this should also work with many settings
# 	print(min_distance)
...
# 	return first_layer, distance_check, min_distance
#
# def find_bundle_boundry_layer_universal_object(helices_pts, neighbour_interactions, distance_threshold=50, search_layer_setting_num=3):
#
# 	#FIXME format docstrings, clean from devel code
# 	#FIXME clone of one function above - change and deal with the prwevious one
# 	#FIXME DEPRECATED - delete it (implemented in searchLayerClass)
#
# 	''' find bundle layer with minimal distance between helices (i.e. points on their axes) '''
# 	''' distance is calculated only between edges, diagonals are not included '''
# 	''' input: list of helices, each helix is list of n 3-element lists of coords, n=res_num_layer_detection '''
...
# 	#print(helices_pts)
#
# 	for h1_idx, h1_point in enumerate(helices_pts):
# 		for h2_idx, h2_point in enumerate(helices_pts):
# 			if (h1_idx != h2_idx and not (h2_idx,h1_idx) in pts_distances.keys()):
# 				dist = distance.cdist(h1_point.coords, h2_point.coords) # Compute distance between each pair of the two collections of inputs.
# 				pts_distances[(h1_idx,h2_idx)] = dist
# 	dist_matrices_number = len(pts_distances.keys()) # number of all possible distances (including diagonals)
#
# 	# mapping of distance id to record in pts_distances
# 	distance_to_pos = { pos:list(pts_distances.keys())[pos] for pos in range(dist_matrices_number)}
...
#
# 	#FIXME DEPRECATED - substituted by find_CA_coresponding_to_point and NOT USED
# 	print('WARNING! USING DEPRECATED FUNCTION find_closest_CA_to_point() from layer_detection')
# 	#FIXME format docstring
#
...
#
# 	helices_CA_array  = convert_Vector_data_to_np_array(helices_CA)
# 	# print('converting axis-> CA')
# 	# for h in helices_CA_array:
# 	# 	print(len(h))
# 	CA_coords         = []
# 	CA_ids            = []
#
...
#
# def find_CA_coresponding_to_point(layer_pts, helices_CA, helices_axis_all_cut_pos, layer_pos):
#
# 	#FIXME format docstring
...
# 	''' output: (CA_ids) list of identifiers (int) of CAs that are closest to desired points, each identifier represents one helix '''
#
# 	def adjust_pos_to_cut_helix(point, helix_axis_all_pos):
#
# 		"""Adjust index of point on the helix axis taking into account:
# 		1) cutting of helix edges (1 point/residue per helix end)
# 		2) cutting of axis points that are too far from points on other helices
# 		"""
#
# 		point_adjust = point + 1 # account for cutting of helix edges
# 		pos_adjust = 0
# 		for pos in helix_axis_all_pos:
# 			if pos == False:
# 				pos_adjust += 1
# 			else:
# 				break
#
# 		point_adjust += pos_adjust
#
# 		return point_adjust
#
# 	helices_CA_array  = convert_Vector_data_to_np_array(helices_CA)
...
# 	# 	print(len(h))
#
# 	# here all CA coords are loaded
# 	# needed: indexes of points that are stored in layer_pts so they can be used to access proper CA
# 	# important: correct for cutting of axis points in socketClass: points indexes must be absolute (full length chain)
#
# 	CA_coords         = []
# 	CA_ids            = []
#
# 	# print('HLX-ALL-POS', helices_axis_all_cut_pos)
# 	# print('POS', layer_pos)
# 	# print('PTS', layer_pts)
#
# 	# przeliczanie: +1 za odciecie koncowek, +1 za kazde False  [dev-docs]
#
# 	for helix_id, point_and_pos in enumerate(zip(layer_pos, helices_axis_all_cut_pos)):
# 		point_adjust = adjust_pos_to_cut_helix(point_and_pos[0], point_and_pos[1])
# 		CA_coords.append(helices_CA_array[helix_id][point_adjust])
# 		CA_ids.append(point_adjust)
#
# 	# for point in enumerate(layer_pts):
# 	# 	dist       = distance.cdist(np.array([point[1]]), helices_CA_array[point[0]])
# 	# 	closest_id = np.argmin(dist)
...
# 	# 	CA_ids.append(closest_id)
#
# 	# print('COORDS-IDS', CA_coords, CA_ids)
#
# 	return CA_coords, CA_ids
#
...
# 	return layers_all
#
# def select_minimal_angle_layer_set_OLDEST(CA_layers_list, layer_ids_list):
#
...
# 	return best_layer_set, best_layer_ids
#
# def select_minimal_distance_layer_set_OLD(CA_layers_list, layer_ids_list):
#
# 	#FIXME rewrite to use new idea for dimers: cast on plane and measure angles between lines connecting helices
# 	#FIXME format docstring
#
